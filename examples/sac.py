...
from nugi_rl.loss.sac_modified.policy_loss import PolicyLoss
from nugi_rl.loss.sac_modified.q_loss import QLoss
...
alpha = 0.1  # Coefficient that control how matters log_prob in policy loss
...
policy_loss = PolicyLoss(distribution)
...
policy_optimizer = torch.optim.AdamW(list(policy.parameters()), lr=learning_rate)
soft_q_optimizer = torch.optim.AdamW(
    list(soft_q1.parameters()) + list(soft_q2.parameters()), lr=learning_rate
)


agent = AgentSACModified(
    soft_q1,
    soft_q2,
    policy,
    distribution,
...
runner = SingleStepRunner(
    agent, environment, is_training_mode, render, plotter, n_plot_batch
)
