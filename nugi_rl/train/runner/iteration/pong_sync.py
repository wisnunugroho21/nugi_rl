    ...
    def run(self) -> tuple:
        ...
        for _ in range(self.n_update):
            ...
            if self.is_save_memory:
                self.agent.save_obs(
                    self.states, action, reward, done, next_state, logprob
                )
            ...
            if done:
                ...
                if self.tag is not None:
                    print(
                        "Episode {} \t t_reward: {} \t eps time: {} \t tag: {}".format(
                            self.i_episode, self.total_reward, self.eps_time, self.tag
                        )
                    )
                else:
                    print(
                        "Episode {} \t t_reward: {} \t eps time: {} ".format(
                            self.i_episode, self.total_reward, self.eps_time
                        )
                    )
