    ...
    def run(self):
        for _ in range(self.n_update):
            ...
            for i in range(self.frame):
                next_obs, reward_temp, done = self.env.step(action_gym)
                next_obs = prepro_half_one_dim(next_obs).unsqueeze(0)

                reward += reward_temp
                next_state = (
                    next_obs if i == 0 else torch.concat((next_state, next_obs), dim=0)
                )

                if done:
                    if next_state.size(0) < self.frame:
                        next_obs = torch.zeros(
                            self.frame - next_state.size(0), 80 * 80, device=self.device
                        )
