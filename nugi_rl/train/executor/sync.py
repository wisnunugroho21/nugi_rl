...
class SyncExecutor(Executor):
    def __init__(
        ...
        learner_device: device = torch.device("cuda"),
    ):
        self.agent = agent
        self.runner = runner
            ...
            for _ in range(self.n_iteration):
                ...
                if self.is_training_mode:
                    for data in datas:
                        ...
                        self.agent.save_all(
                            states.to(self.learner_device),
                            actions.to(self.learner_device),
                            rewards.to(self.learner_device),
                            dones.to(self.learner_device),
                            next_states.to(self.learner_device),
                            logprobs.to(self.learner_device),
                        )
