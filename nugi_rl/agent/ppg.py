...
from nugi_rl.memory.state.ppg import PPGMemory
...
class AgentPPO(Agent):
    ...
    def _update_policy(self) -> None:
        ...
        states, _, _, _, _, _ = self.policy_memory.get()
    ...
    def load_weights(self) -> None:
        model_checkpoint = torch.load(
            self.folder + "/ppg.tar", map_location=self.device
        )
        ...
        if self.policy_optimizer is not None:
            self.policy_optimizer.load_state_dict(
                model_checkpoint["ppo_optimizer_state_dict"]
            )

        if self.aux_optimizer is not None:
            self.aux_optimizer.load_state_dict(
                model_checkpoint["aux_ppg_optimizer_state_dict"]
            )
