...
from nugi_rl.memory.state.clr import ClrMemory
...
class AgentImagePpoClr(AgentPPO):
    def __init__(
        ...
        aux_clr_memory: ClrMemory,
        optimizer: Optimizer,
        aux_clr_optimizer: Optimizer,
        trans: Augmentation,
        ppo_epochs: int = 10,
        aux_clr_epochs: int = 5,
        ...
        cnn_old: Module | None = None,
        projector_old: Module | None = None,
    ...
    def _update_aux_clr(self) -> None:
        ...
        for _ in range(self.aux_clr_epochs):
            dataloader = DataLoader(
                self.aux_clr_memory, self.batch_size, shuffle=True, num_workers=8
            )
    ...
    def load_weights(self) -> None:
        model_checkpoint = torch.load(
            self.folder + "/ppg.pth", map_location=self.device
        )
        ...
        if self.aux_clr_optimizer is not None:
            self.aux_clr_optimizer.load_state_dict(
                model_checkpoint["aux_clr_optimizer_state_dict"]
            )
