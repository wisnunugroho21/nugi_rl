...
class SeparableConv2d(nn.Module):
    def __init__(self, nin, nout, kernel_size = 3, stride = 1, padding = 0, dilation = 1, bias = True, depth_multiplier = 1):
        ...
        self.nn_layer = nn.Sequential(
            nn.Conv2d(nin, nout * depth_multiplier, kernel_size = 1, bias = bias),
            nn.Conv2d(nout * depth_multiplier, nout, kernel_size = kernel_size, stride = stride, padding = padding, dilation = dilation, bias = bias, groups = nout)            
        )
