...
class ContinuousAlphaLoss(AlphaLoss):
    def __init__(
        ...
        coef_alpha_mean_upper: Tensor = torch.Tensor([0.01]),
        coef_alpha_mean_below: Tensor = torch.Tensor([0.005]),
        coef_alpha_cov_upper: Tensor = torch.Tensor([0.01]),
        coef_alpha_cov_below: Tensor = torch.Tensor([0.005]),
    ):
        super().__init__()
    ...
    def forward(
        ...
        coef_mean_alpha = (
            torch.distributions.Uniform(
                self.coef_alpha_mean_below.log(), self.coef_alpha_mean_upper.log()
            )
            .sample()
            .exp()
        )
        coef_cov_alpha = (
            torch.distributions.Uniform(
                self.coef_alpha_cov_below.log(), self.coef_alpha_cov_upper.log()
            )
            .sample()
            .exp()
        )
        ...
        mean_loss = (
            alpha_mean * (coef_mean_alpha - Kl_mean.squeeze().detach())
            + alpha_mean.detach() * Kl_mean.squeeze()
        )
        cov_loss = (
            alpha_cov * (coef_cov_alpha - Kl_cov.squeeze().detach())
            + alpha_cov.detach() * Kl_cov.squeeze()
        )
