...
class DiscreteAlphaLoss(AlphaLoss):
    def __init__(
        ...
        coef_alpha_upper: Tensor = torch.Tensor([0.01]),
        coef_alpha_below: Tensor = torch.Tensor([0.005]),
    ):
        super().__init__()
    ...
    def forward(
        ...
        coef_alpha = (
            torch.distributions.Uniform(
                self.coef_alpha_below.log(), self.coef_alpha_upper.log()
            )
            .sample()
            .exp()
        )
        ...
        loss = (
            alpha * (coef_alpha - Kl.squeeze().detach()) + alpha.detach() * Kl.squeeze()
        )
        return loss.mean()
