...
class Ordinal(Distribution):
    ...
    def _compute_ordinal(self, logits: Tensor) -> Tensor:
        ...
        a1 = (
            torch.ones(logits.size(2), logits.size(2), device=self.device)
            .triu()
            .transpose(0, 1)
            .repeat(logits.size(0), logits.size(1), 1, 1)
        )
    ...
    def sample(self, datas: Tensor) -> Tensor:
        logits = self._compute_ordinal(datas)
    ...
    def kldivergence(self, datas1: Tensor, datas2: Tensor) -> Tensor:
        logits1 = self._compute_ordinal(datas1)
        logits2 = self._compute_ordinal(datas2)
    ...
    def deterministic(self, data: Tensor) -> Tensor:
        logits = self._compute_ordinal(data)
