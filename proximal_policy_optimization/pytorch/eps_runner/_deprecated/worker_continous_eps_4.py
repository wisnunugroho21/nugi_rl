...
def get_action(state):
    ...
    data = r.json()
    return data['action'], data['logprob']

def send_trajectory(states, actions, rewards, dones, next_states, logprobs, next_next_states):
    data = {
        'states'            : states,
        'actions'           : actions,
        'rewards'           : rewards,        
...
def run_an_episode(memory, env, render, training_mode, max_action):
    ...
    for _ in range(1, 5000): 
        action, logprob = get_action(state.tolist())
        ...
        if training_mode: 
            memory.save_eps(state.tolist(), action, reward, float(done), next_state.tolist(), logprob)
