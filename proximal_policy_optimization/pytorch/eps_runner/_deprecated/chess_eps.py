...
def run_an_episode(env, agent, render, training_mode, t_updates, n_update, params, params_max, params_min, params_subtract, params_dynamic):
    ...
    ############################################
    state = env.reset() 
    possible_actions = env.get_possible_actions(state, player)
    state = state['board']
    ...
    for _ in range(10000): 
        action = int(agent.act(state, possible_actions))       
        next_state, reward, done, _ = env.step(action)

        possible_actions = env.get_possible_actions(next_state, player)
        ...
        if training_mode: 
            agent.save_eps(state.tolist(), reward, action, float(done), next_state.tolist(), possible_actions)             
