...
class Agent:  
    ...
    def training_ppo(self, states, actions, rewards, dones, next_states, worker_logprobs):
        ...
        loss = get_loss(action_probs, old_action_probs, values, old_values, next_values, actions, rewards, dones, worker_logprobs,
    ...
    def update_ppo(self, memory = None): 
        ...
        batch_size = int(len(memory) / self.minibatch)
        dataloader = DataLoader(memory, batch_size, shuffle = False)
        
        # Optimize policy for K epochs:
        for _ in range(self.PPO_epochs):       
            for states, actions, rewards, dones, next_states, worker_logprobs in dataloader:
                self.training_ppo(states.float().to(self.device), actions.float().to(self.device), rewards.float().to(self.device), dones.float().to(self.device), next_states.float().to(self.device), worker_logprobs.float().to(self.device))
    ...
    def log_action(self):
        states, actions, _, _, _, _     = self.memory.get_all_items()
        states, actions                 = torch.FloatTensor(states).to(set_device(self.use_gpu)), torch.FloatTensor(actions).unsqueeze(1).to(set_device(self.use_gpu))
        action_mean                     = self.actor(states)

        return logprob(action_mean, self.action_std, actions, self.use_gpu)