...
def run_an_episode(env, agent, render, training_mode, t_updates, n_update, params, params_max, params_min, params_subtract, params_dynamic, max_action):
    ...
    for _ in range(1, 5000): 
        ...
        if training_mode:
            agent.memory.save_eps(state.tolist(), action.numpy(), reward, float(done), next_state.tolist()) 
...
def run_continous(agent, env, n_episode, reward_threshold, save_weights = False, n_plot_batch = 100, render = True, training_mode = True, n_update = 1024, n_saved = 10,
