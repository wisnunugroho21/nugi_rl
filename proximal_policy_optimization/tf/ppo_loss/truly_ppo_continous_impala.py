...
from distribution.multivariate_normal_distribution import sample, entropy, kldivergence, logprob
...
# Loss for PPO  
def get_loss(action_mean, worker_action_mean, old_action_mean, values, old_values, next_values, actions, rewards, dones,
        ...
        Worker_logprobs = tf.stop_gradient(logprob(worker_action_mean, action_std, actions))
        ...
        ratios          = tf.math.exp(logprobs - Old_logprobs) * tf.math.exp(logprobs - Worker_logprobs) # ratios = old_logprobs / logprobs        
        Kl              = kldivergence(old_action_mean, action_std, action_mean, action_std) + kldivergence(worker_action_mean, action_std, action_mean, action_std)
