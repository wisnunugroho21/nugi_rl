...
class CnnModel(nn.Module):
    def __init__(self):
      ...
      self.conv3 = nn.Sequential(
        DepthwiseSeparableConv2d(16, 64, kernel_size = 8, stride = 4, padding = 2),
        nn.ReLU(),
      )
...
class DecoderModel(nn.Module):
    def __init__(self):
      ...
      self.conv3 = nn.Sequential(
        ...
        DepthwiseSeparableConv2d(64, 16, kernel_size = 3, stride = 1, padding = 1),
        nn.ReLU(),
      )
...
class Policy_Model(nn.Module):
    def __init__(self, state_dim, action_dim, use_gpu = True):
      ...
      self.memory_layer   = nn.LSTM(128, 128).float().to(set_device(use_gpu))
      self.nn_layer       = nn.Sequential( nn.Linear(128, 64), nn.ReLU() ).float().to(set_device(use_gpu))
